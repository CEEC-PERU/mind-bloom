...
export const AuthProvider = ({ children }: any) => {
  ...
  useEffect(() => {
    const handleAppStateChange = async (nextAppState: string) => {
      if (nextAppState === 'background') socket.emit('logout');
    };
    ...
    (async () => {
      try {
        ...
        if (storedUserInfo && isValid) {
          const storedProfileInfo = await SecureStore.getItemAsync(
            'profileInfo'
          );
          if (storedProfileInfo) setProfileInfo(JSON.parse(storedProfileInfo));
          const { id, role, email, client_id } = JSON.parse(storedUserInfo);
          setUserToken(storedUserToken);
          setUserInfo({ id, role, email, client_id });
          if (role === 1) socket.emit('login', { userToken: storedUserToken });
        } else {
          setUserToken(null);
          setUserInfo(null);
        }
